...
function dijkstraConRetardos(red, origen, destino) {
    ...
    while (nodosNoVisitados.size > 0) {
      ...
      const auxNode = red.find(n => n.id === nodoActual);
      for (const { nodo, peso } of auxNode.conexiones) {
        const distanciaTotal = distancia[nodoActual] + peso + nodo.retardo; // Considerar el retardo en el procesamiento del nodo
    ...
    while (nodoActual !== null && nodoActual !== undefined) {
      rutaMinima.unshift(red.find(nodo => nodo.id === nodoActual));
